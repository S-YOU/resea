...
struct kfs_file *kfs_readdir(struct kfs_dir *dir, struct kfs_file *file) {
    INFO("kfs: '%s' %p", dir->current->name, dir->current);
    MAGICBREAK
    ...
    file->data = (void *) ((uintptr_t) dir->current + sizeof(struct kfs_file_header));
    file->pager_arg = dir->current;

    size_t len = sizeof(struct kfs_file_header) + dir->current->length;
    dir->current = (struct kfs_file_header *) ((uintptr_t) dir->current + len);
    return file;
}
